...
>
> **修订说明（2026-08-28 · T-08 改名）**：本规格正文内 `CalcRefundable` 为**设计稿曾用名**，代码落地的现名 = `CalcRemaining`。FR-4.1 函数签名 / FR-7.2 Bug2 描述 / FR-8.3 坏味道锚点 / AC-10 / AC-31 / 约束条款等处保留曾用名，以保「设计 → 代码」追溯链；读者按 NFR-7 配方 `rg CalcRemaining` 查调用点即可定位现名实现。
...
| NFR-7 | 教学可观测性 | S-A 探索时，入口 main.go → routes → domain → db 的调用链路是否无隐式魔法 | 100% 直白可见；不用看文档就能通过 `rg CalcRemaining` 找到全部调用点 |
...
>
> ① **运费怎么退？** 顾客实际付的运费，`shipping` 字段里有，退的时候要和货款一起算进「可退上限」。*（TODO-① 对应 refund_rules.go 顶部第 1 条：运费是否计入退款基数）*
>
> ② **用了优惠券的订单怎么退？** 顾客用掉的 `coupon_used` 已经抵了货款，**退的时候要先扣掉**（也就是可退上限 = `amount + shipping − coupon_used`），别让人家用券还能白赚回来。*（TODO-② 对应 refund_rules.go 顶部第 2 条：用券返法——退时先扣抵用额）*
>
> ③ **顾客申请超过可退上限怎么办？** 分两种理解都能走，但必须只选一种保持一致：
...
### C-2 B2 话术·老王 Bug（对应 b2-bug 分支）
> 【后端 · 老王 02:07】紧急！线上 0 元的赠品单居然能建，然后顾客一顿申请退款，订单直接 400；另外退 3000 显示剩 7000，我再申请 5000 居然说「已退满」——你们看一下 `internal/domain/refund_rules.go` 里是不是 `ReverseRefundRemaining` 写反了？订单金额门槛也看看 `internal/routes/orders.go` `OrderMinAmount` 现在是不是 0 了？

### C-3 B3 话术·老王 重构咖啡时间（对应 main 分支练习）
> 【后端 · 老王 咖啡时间】`internal/routes/orders.go` 和 `refunds.go` 的 Handler 样板重复得我手痒，`schema.sql` 两表挤一个文件，`CalcRemaining` 函数超长——这三处重构，**契约和测试 diff 必须为 0**，你们搞一下。
